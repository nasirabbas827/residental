...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty(trim($_POST["username"]))) {
        ...
        $sql = "SELECT id FROM tenants WHERE username = ?";
    ...
    if (empty($username_err) && empty($password_err) && empty($email_err) && empty($phone_err) && empty($age_err)) {
        $sql = "INSERT INTO tenants (username, password, email, phone, age) VALUES (?, ?, ?, ?, ?)";
        $stmt = mysqli_prepare($conn, $sql);
        mysqli_stmt_bind_param($stmt, "sssis", $param_username, $param_password, $param_email, $param_phone, $param_age);
...
<div class="container mt-5">
    <h2 class="text-center">Tenants Registration</h2>
